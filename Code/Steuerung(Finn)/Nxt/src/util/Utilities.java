...
public class Utilities {
	...
	public static String replaceAll(String toEdit, String regex, String replacement) {
		if(contains(toEdit, regex)){
            StringBuilder sb = new StringBuilder();
            if (toEdit.equals(regex))
                return regex;

            if (toEdit.startsWith(regex))
                sb.append(replacement);

            for (String s : split(toEdit, regex)) {
                sb.append(s).append(replacement);
            }

            if (sb.length() > 0)
                sb.setLength(sb.length() - replacement.length());

            if (toEdit.length() > regex.length() && toEdit.substring(toEdit.length() - regex.length()).equals(regex))
                sb.append(replacement);

            return ((sb.length() > 0) ? sb.toString() : toEdit);
        }
        return toEdit;
	}
		...
		if (!wait) {
			for (NXTMessage m : messages) {
				if (m.equals(NXTMessage.clearLCD)) {
					...
					LCD.drawString(m.getValues()[0] + "", m.getValues()[1], m.getValues()[2]);
				} else if (m.equals(NXTMessage.setSpeed)) {
					Motors.setSpeed(m.getValues()[0], m.getValues()[1]);
				} else if (m.equals(NXTMessage.rotate)) {
					Motors.rotate(m.getValues()[0], m.getValues()[1]);
				} else if (m.equals(NXTMessage.waitTillFinished)) {
					waitTillFinished();
				} else if (m.equals(NXTMessage.runSyncTask) & !syncTask) {
					runSyncTask(m.getValues()[0], messages);
					wait = true;
					break;
					...
					Motors.rotateTo(m.getValues()[0], m.getValues()[1]);
